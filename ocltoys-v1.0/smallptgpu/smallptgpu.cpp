...
protected:
	...
	virtual void DisplayCallBack() {
	    ...
	    if (selectedDevices.size() == 1)
	        glTexSubImage2D(TextureTarget, 0, 0, 0, windowWidth, windowHeight, TextureFormat, TextureType, pixels[0]);
	    else {
	    	// Multiple devices, I have to merge the results and to apply tone mapping
			const unsigned count = windowWidth * windowHeight * 3;
			std::copy(pixels[0], pixels[0] + count, mergedPixels);
			...
			glTexSubImage2D(TextureTarget, 0, 0, 0, windowWidth, windowHeight, TextureFormat, TextureType, mergedPixels);
	    }
	    ...
	    glColor3f( 1, 1, 1 );
	    glTexCoord2i( 0, 1 ); glVertex3f( 0, windowHeight, 0 );
	    glColor3f( 1, 1, 1 );
	    glTexCoord2i( 1, 0 ); glVertex3f( windowWidth, 0, 0 );
	    glColor3f( 1, 1, 1 );
	    glTexCoord2i( 1, 1 ); glVertex3f( windowWidth, windowHeight, 0 );
	    glEnd();
...
private:
	...
	void PrintHelp() {
		...
		fontOffset -= 17;
		PrintHelpString(60, fontOffset, "+/1 and -/0", "to select next/previous object");
		fontOffset -= 17;
		...
		for (unsigned int i = 0; i < selectedDevices.size(); ++i) {
			...
			const std::string deviceString = boost::str(boost::format("  [%s][%.1fM Sample/sec]") %
				selectedDevices[deviceIndex].getInfo<CL_DEVICE_TYPE>() % (sampleSec[deviceIndex] / 1000000.0));
	...
	static void RenderThreadImpl(SmallPTGPU *smallptgpu, const unsigned int threadIndex) {
		try {
			...
			static unsigned int kernelIterations = 1;
			if (smallptgpu->renderThreads[threadIndex] == 0) {
				smallptgpu->renderThreads[threadIndex] = 1;
			#else
